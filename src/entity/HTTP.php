...
class HTTP
{
    ...
    {
        // 传参请求
        $request = $this->request($url, $params, 'POST', $options);
        return $request;
    }
    ...
    {
        // 传参请求
        $request = $this->request($url, $params, 'GET', $options);
        return $request;
    }
     ...
     * @return void
     */
    public function download(String $url, String $dir, String $name, array $params = [], String $method = 'GET', array $options = [])
    {
        try {
            // 判断是否需要创建目录
            ...
            $tmpFile = $dir . $name;
            // 请求远程数据
            $result = $this->request($url, $params, $method, $options);
            // 开始写入文件
     ...
     * @return boolean TRUE
     */
    public function asyncRequest(String $url, $params = [], String $method = 'POST')
    {
        // 转换大写
        $method = strtoupper($method);
        // 仅允许POST\GET请求
     ...
     */
    public function toBrowser($file, $exitAfterSend = true)
    {
        // 判断文件是否存在以及可读
